        ...
        try:
            ...
            if model_file!=None:
                if backend in ontologyConstants.TENSORFLOW_URIS:
                    ...
                    return "This method only supports Tensorflow/Keras or PyTorch models.",BAD_REQUEST
            else:
            ...
            if "target_layer_index" in params_json:
                try:
                    ...
                    return "The specified index could not be accessed in the target_layer." ,BAD_REQUEST
            ...
            if(is_tf):
                ...
                if "target_layer" in params_json:
                    if hasattr(mlp,params_json["target_layer"]):
                        ...
                        return "The specified target layer " + str(params_json["target_layer"]) + " does not exist.",BAD_REQUEST
                else:
                ...
                if instance!=None:
                    try:
                        ...
                        return "Could not read instance from JSON.",BAD_REQUEST
                elif image!=None:
                    try:
                        im = Image.open(image)
                    except:
                        return "Could not load image from file.",BAD_REQUEST
                    try:
                        transform = transforms.Compose([transforms.ToTensor()])
                        input_tensor=transform(im.copy())
                    except Exception as e:
                        print(e)
                        return "Could not convert image to Tensor.",BAD_REQUEST
                else:
                    return "Either an image file or a matrix representative of the image must be provided.",BAD_REQUEST
